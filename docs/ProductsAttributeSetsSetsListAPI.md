...
------------- | ------------- | -------------
[**GetV1ProductsAttributesetsSetsList**](ProductsAttributeSetsSetsListAPI.md#GetV1ProductsAttributesetsSetsList) | **Get** /V1/products/attribute-sets/sets/list | products/attribute-sets/sets/list
...
func main() {
	...
	resp, r, err := apiClient.ProductsAttributeSetsSetsListAPI.GetV1ProductsAttributesetsSetsList(context.Background()).SearchCriteriaFilterGroups0Filters0Field(searchCriteriaFilterGroups0Filters0Field).SearchCriteriaFilterGroups0Filters0Value(searchCriteriaFilterGroups0Filters0Value).SearchCriteriaFilterGroups0Filters0ConditionType(searchCriteriaFilterGroups0Filters0ConditionType).SearchCriteriaSortOrders0Field(searchCriteriaSortOrders0Field).SearchCriteriaSortOrders0Direction(searchCriteriaSortOrders0Direction).SearchCriteriaPageSize(searchCriteriaPageSize).SearchCriteriaCurrentPage(searchCriteriaCurrentPage).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ProductsAttributeSetsSetsListAPI.GetV1ProductsAttributesetsSetsList``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetV1ProductsAttributesetsSetsList`: EavDataAttributeSetSearchResultsInterface
	fmt.Fprintf(os.Stdout, "Response from `ProductsAttributeSetsSetsListAPI.GetV1ProductsAttributesetsSetsList`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetV1ProductsAttributesetsSetsListRequest struct via the builder pattern
