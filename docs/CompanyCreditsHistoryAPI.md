...
------------- | ------------- | -------------
[**GetV1CompanyCreditsHistory**](CompanyCreditsHistoryAPI.md#GetV1CompanyCreditsHistory) | **Get** /V1/companyCredits/history | companyCredits/history



## GetV1CompanyCreditsHistory

> CompanyCreditDataHistorySearchResultsInterface GetV1CompanyCreditsHistory(ctx).SearchCriteriaFilterGroups0Filters0Field(searchCriteriaFilterGroups0Filters0Field).SearchCriteriaFilterGroups0Filters0Value(searchCriteriaFilterGroups0Filters0Value).SearchCriteriaFilterGroups0Filters0ConditionType(searchCriteriaFilterGroups0Filters0ConditionType).SearchCriteriaSortOrders0Field(searchCriteriaSortOrders0Field).SearchCriteriaSortOrders0Direction(searchCriteriaSortOrders0Direction).SearchCriteriaPageSize(searchCriteriaPageSize).SearchCriteriaCurrentPage(searchCriteriaCurrentPage).Execute()
...
func main() {
	...
	resp, r, err := apiClient.CompanyCreditsHistoryAPI.GetV1CompanyCreditsHistory(context.Background()).SearchCriteriaFilterGroups0Filters0Field(searchCriteriaFilterGroups0Filters0Field).SearchCriteriaFilterGroups0Filters0Value(searchCriteriaFilterGroups0Filters0Value).SearchCriteriaFilterGroups0Filters0ConditionType(searchCriteriaFilterGroups0Filters0ConditionType).SearchCriteriaSortOrders0Field(searchCriteriaSortOrders0Field).SearchCriteriaSortOrders0Direction(searchCriteriaSortOrders0Direction).SearchCriteriaPageSize(searchCriteriaPageSize).SearchCriteriaCurrentPage(searchCriteriaCurrentPage).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CompanyCreditsHistoryAPI.GetV1CompanyCreditsHistory``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetV1CompanyCreditsHistory`: CompanyCreditDataHistorySearchResultsInterface
	fmt.Fprintf(os.Stdout, "Response from `CompanyCreditsHistoryAPI.GetV1CompanyCreditsHistory`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetV1CompanyCreditsHistoryRequest struct via the builder pattern
