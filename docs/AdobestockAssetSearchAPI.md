# \AdobestockAssetSearchAPI

All URIs are relative to *http://example.com/rest/default*

Method | HTTP request | Description
------------- | ------------- | -------------
[**GetV1AdobestockAssetSearch**](AdobestockAssetSearchAPI.md#GetV1AdobestockAssetSearch) | **Get** /V1/adobestock/asset/search | adobestock/asset/search
...
func main() {
	...
	resp, r, err := apiClient.AdobestockAssetSearchAPI.GetV1AdobestockAssetSearch(context.Background()).SearchCriteriaFilterGroups0Filters0Field(searchCriteriaFilterGroups0Filters0Field).SearchCriteriaFilterGroups0Filters0Value(searchCriteriaFilterGroups0Filters0Value).SearchCriteriaFilterGroups0Filters0ConditionType(searchCriteriaFilterGroups0Filters0ConditionType).SearchCriteriaSortOrders0Field(searchCriteriaSortOrders0Field).SearchCriteriaSortOrders0Direction(searchCriteriaSortOrders0Direction).SearchCriteriaPageSize(searchCriteriaPageSize).SearchCriteriaCurrentPage(searchCriteriaCurrentPage).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `AdobestockAssetSearchAPI.GetV1AdobestockAssetSearch``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetV1AdobestockAssetSearch`: AdobeStockAssetApiDataAssetSearchResultsInterface
	fmt.Fprintf(os.Stdout, "Response from `AdobestockAssetSearchAPI.GetV1AdobestockAssetSearch`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetV1AdobestockAssetSearchRequest struct via the builder pattern
...
[**AdobeStockAssetApiDataAssetSearchResultsInterface**](AdobeStockAssetApiDataAssetSearchResultsInterface.md)
