...
------------- | ------------- | -------------
[**GetV1CustomersSearch**](CustomersSearchAPI.md#GetV1CustomersSearch) | **Get** /V1/customers/search | customers/search
...
func main() {
	...
	resp, r, err := apiClient.CustomersSearchAPI.GetV1CustomersSearch(context.Background()).SearchCriteriaFilterGroups0Filters0Field(searchCriteriaFilterGroups0Filters0Field).SearchCriteriaFilterGroups0Filters0Value(searchCriteriaFilterGroups0Filters0Value).SearchCriteriaFilterGroups0Filters0ConditionType(searchCriteriaFilterGroups0Filters0ConditionType).SearchCriteriaSortOrders0Field(searchCriteriaSortOrders0Field).SearchCriteriaSortOrders0Direction(searchCriteriaSortOrders0Direction).SearchCriteriaPageSize(searchCriteriaPageSize).SearchCriteriaCurrentPage(searchCriteriaCurrentPage).Execute()
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `CustomersSearchAPI.GetV1CustomersSearch``: %v\n", err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
	}
	// response from `GetV1CustomersSearch`: CustomerDataCustomerSearchResultsInterface
	fmt.Fprintf(os.Stdout, "Response from `CustomersSearchAPI.GetV1CustomersSearch`: %v\n", resp)
}
```
...
Other parameters are passed through a pointer to a apiGetV1CustomersSearchRequest struct via the builder pattern
